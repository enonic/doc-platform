...
Basic authentication on the management endpoint is now an `autoLogin` capability of the Standard ID Provider application, scoped to the system ID provider on that endpoint. It stays enabled by default and can be disabled with a single config line. See <<endpoints/management#authorization, management API authorization>>.
