...
Throughout this guide, we will describe how to set up a <<./strategies#basic_cluster,basic cluster>>, but with little more effort you could create any type of cluster. A __basic cluster__ consists of 3 dedicated XP _master_ nodes and 2 XP _combined_ nodes.
...
Since Elasticsearch is embedded in XP, be sure also to set up virtual memory, file descriptors and swap setting correctly for __elasticsearch__. Read more about that in the https://www.elastic.co/guide/en/elasticsearch/reference/2.4/setup-configuration.html[elasticsearch documentation].
...
A clustered deployment needs a load balancer in front of the nodes to distribute requests. Load balancing — including session affinity (session replication versus sticky sessions) and health checks — is covered in <<edge#load_balancing, Web server, load balancing and CDN>>.
...
TIP: It is possible to free master nodes from running named tasks by setting `distributable.acceptInbound = false` in <<../config/system#task, Task>> config.

TIP: image:../../images/xp-810.svg[XP 8.1.0,opts=inline] Master nodes can also be excluded from running the scheduler, by setting `acceptScheduling = false` in <<../config/system#scheduler, Scheduler>> config. This is a separate choice from `distributable.acceptInbound` above: that one governs where the tasks run, this one where the schedule that submits them runs. Both are worth setting on a dedicated master node, so that neither the jobs nor the tasks they start can put a master node's quorum duty at risk.
