...
TIP: image:xp-8010.svg[XP 8.1.0,opts=inline] Which node runs the scheduler is a separate choice from which nodes run the tasks it submits, and is controlled by `acceptScheduling` in <<../config/system.adoc#scheduler, Scheduler>> config. The scheduler itself is light - once a second it checks whether a job is due and submits a task - but it wants a node that stays up, since scheduling moves elsewhere as soon as that node leaves. Setting `acceptScheduling = false` keeps the schedule off a node without affecting the scheduler API, which every node still serves.
