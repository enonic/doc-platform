...
Routing:: Forward incoming requests to XP's web endpoint. In a clustered deployment this also means distributing requests across the available nodes — see <<load_balancing, Load balancing>>.
+
...
NOTE: This is a getting-started baseline for a single node. Production deployments are typically clustered and add load balancing across several nodes — see <<load_balancing, Load balancing>>.
