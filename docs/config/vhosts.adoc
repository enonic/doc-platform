...
Any other name in the list addresses an additional flow of the ID provider application itself - for instance, the Standard ID Provider serves basic authentication only where `basic` is listed, and an ID provider may serve its device authorization endpoints only where `device` is listed. See the ID provider's own documentation for the flows it supports.

Without a flow list, no restriction applies: the ID provider serves whatever flows it supports.
