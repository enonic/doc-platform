...
NOTE: Vhosts apply to XP's web endpoint (defaults to port 8080) unless a mapping names another endpoint. image:xp-820.svg[XP 8.2.0,opts=inline] A mapping may also target the <<#endpoint, management or statistics endpoint>>, bringing ID provider authentication with it.
...
idProvider:: optionally adds one or more idProviders to the vhost. `idProvider` must be followed by the name of an existing idProvider. The example above refers to the idProvider called `myldap`. Supported values are `default` or `enabled`. Only one entry may use `default`. image:xp-820.svg[XP 8.2.0,opts=inline] `enabled` may carry a <<#flows, flow list>>: `default&enabled=login,autologin`.
endpoint:: image:xp-820.svg[XP 8.2.0,opts=inline] optionally targets the mapping at the <<#endpoint, management or statistics endpoint>> instead of the web endpoint.
allow:: image:xp-820.svg[XP 8.2.0,opts=inline] optionally restricts the vhost to a <<#allow, list of principals>>.
...
image:xp-820.svg[XP 8.2.0,opts=inline] The flows an ID provider serves on a vhost can be restricted with a flow list on the `enabled` value:
...
image:xp-820.svg[XP 8.2.0,opts=inline] A mapping applies to the web endpoint unless another endpoint is named explicitly:
...
Mappings never apply across endpoints: a web mapping stays on the web endpoint. An endpoint no mapping names keeps behaving as if vhosts were not configured; once at least one mapping names the management or statistics endpoint, requests there whose host matches none of its mappings are rejected with 404.
...
[#api-verbs]
=== Per-API restrictions

A management mapping can also narrow what each <<../endpoints/management#, management API>> exposes, through <<#virtual-host-context, context attributes>> the APIs read. The one setting every management API understands is `verbs`: the comma separated list of operations the vhost exposes, `*` for all. An absent setting means no restriction.

[source,properties]
----
mapping.mgmt.context.api.server\:snapshot.verbs = list, create
mapping.mgmt.context.api.server\:task.verbs = *
----

[#allow]
== Allow list

image:xp-820.svg[XP 8.2.0,opts=inline] `allow` restricts who may pass through the vhost. The list is checked after authentication and before the request reaches anything else: unauthenticated requests are rejected with 401, authenticated ones without any of the listed principals with 403.
