...
NOTE: A mapping applies to XP's web endpoint (port 8080) unless its `endpoint` says otherwise - see <<#endpoints,Management and statistics endpoints>>.
...
endpoint:: image:xp-8020.svg[XP 8.2.0,opts=inline] the endpoint the mapping applies to: `web` (default), `management` or `statistics`. See <<#endpoints,Management and statistics endpoints>>.
allow:: image:xp-8020.svg[XP 8.2.0,opts=inline] comma separated principals (`role:system.admin`, `user:system:ops`, ...) that may pass through the mapping. Empty means no restriction.
order:: mappings with a lower value are matched first. See <<#_request_matching,Request Matching>>.
...
image:xp-8020.svg[XP 8.2.0,opts=inline] On a `management` mapping, the `api.` context namespace selects what the <<../endpoints/management/resources#,management resources>> expose through it. Each resource reads its own settings from the request context and enforces them; a setting's meaning is defined by the resource. Every resource understands `verbs`: the comma separated operations the mapping exposes.

[source,properties]
----
mapping.backup.host = backup.example.com
mapping.backup.source = /
mapping.backup.target = /
mapping.backup.endpoint = management
mapping.backup.allow = user:system:backup-agent
mapping.backup.context.api.rest.enabled = false                    <1>
mapping.backup.context.api.server\:snapshot.verbs = list, create   <2>
mapping.backup.context.api.server\:dump.verbs = list, create
mapping.backup.context.api.server\:task.verbs = *                  <3>
----

<1> Switches the <<../endpoints/management#,legacy REST endpoints>> (`repo/snapshot/list`, `system/dump`, ...) off for this mapping, leaving only the `+/<application>:<api>+` resources. Default: enabled.
<2> `server:snapshot` answers `list` and `create` on this mapping and refuses `restore` and `prune` with `403`. Note the escaped colon: the file is read as a Java properties file, where an unescaped colon ends the key.
<3> `*` exposes every verb. A resource without a `verbs` setting is unrestricted: `server:app`, `server:index`, `server:system`, ... remain fully available on this mapping. `+mapping.backup.context.api.server\:app.verbs = -+` exposes no verb of a resource.

A path alias (`+/server:system/vacuum+` for `+/server:system/prune+`, `+/server:app/installUrl+` for `+/server:app/pull+`) belongs to the verb it aliases and is exposed exactly when that verb is.

`verbs` decides only whether a verb is reachable through the mapping. What a verb is allowed to do is configured by the resource itself and is not a virtual host setting: `server:app` `pull`, for example, enforces the allow-list and checksum policy in <<system#management-app,`com.enonic.xp.server.management.app.cfg`>> on every mapping that exposes it.

Keys under `api.` are validated when the file is loaded. A key that is neither `api.rest.enabled` nor `+api.<application>:<api>.<setting>+` (typically a colon that was not escaped) is a configuration error and the file is rejected.

Resources may define further settings in the same namespace. A Universal API of an installed application mounted on the management endpoint reads its own `+api.<application>:<api>.<setting>+` attributes from the context with `contextLib` - see <<https://developer.enonic.com/docs/xp/stable/code/web/apis#management-endpoint,Universal APIs on the management endpoint>>.

[#api-locations]
=== API locations

image:xp-810.svg[XP 8.1.0,opts=inline] APIs may be served from somewhere other than the address a request arrives on: a dedicated API host, a shorter path, or a media host. The context of a virtual host mapping declares where, for requests it matches:
...
image:xp-8020.svg[XP 8.2.0,opts=inline] A mapping applies to the web endpoint unless `endpoint` names the `management` (port 4848) or `statistics` (port 2609) endpoint. Host and source matching, `target` rewriting, id providers and `context` work the same on every endpoint; `allow` restricts who may pass through.

.The management endpoint behind a host name, for administrators only
[source,properties]
----
mapping.mgmt.host = mgmt.example.com
mapping.mgmt.source = /
mapping.mgmt.target = /
mapping.mgmt.endpoint = management
mapping.mgmt.idProvider.myidprovider = default&enabled=autologin
mapping.mgmt.allow = role:system.admin
----

Once vhosts are enabled, a request on the management or statistics endpoint that no mapping matches is served as before; only the web endpoint refuses unmapped requests. A management mapping selects what it exposes with the <<#management-api-policy,management API policy>> in its context.
