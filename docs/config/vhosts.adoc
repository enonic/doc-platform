...
Any other name in the list addresses an additional flow of the ID provider application itself - for instance, an ID provider may serve its device authorization endpoints only where `device` is listed. See the ID provider's own documentation for the flows it supports.
