...
Generated media URLs then point at the CDN, and `+<site>/_/media:image/…+` is no longer served — so the CDN must reach the media APIs through XP's own `+/api/media:image/…+` space. Disabling the auto-mount *without* setting a default base URL instead makes generated URLs use that `+/api/+` form directly.
