...
legacy.mediaApiAutoMount.enabled:: image:xp-8010.svg[XP 8.1.0,opts=inline] Enables/Disables implicit mounting of the media APIs on every site. Default `true` - means every site serves `+/_/media:image/+` and `+/_/media:attachment/+` without declaring them in its site descriptor. Set to `false` to require each site to mount them explicitly. See <<media_base_url, Serving media from another host>>.
media.defaultBaseUrl:: image:xp-8010.svg[XP 8.1.0,opts=inline] Base URL for generated media (image, attachment) URLs, so media can be served from a separate host such as a CDN. Only consulted where the mount handling the request does not serve the media APIs itself. Default: empty - media URLs stay anchored to the incoming request. See <<media_base_url, Serving media from another host>>.
