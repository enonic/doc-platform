...
xp.script-engine:: image:preview.svg[Preview,opts=inline] The default JavaScript engine for applications, `Nashorn` or `GraalJS`. An application overrides it with the `+X-Script-Engine+` header in its bundle manifest. Default: `Nashorn`. See <<#script-engine, JavaScript engine>> below.

[#script-engine]
=== JavaScript engine

image:preview.svg[Preview,opts=inline] XP runs server-side JavaScript on one of two engines. *Nashorn* is the default and the supported engine. *GraalJS* ships alongside it as a preview: it implements the current ECMAScript standard rather than Nashorn's ES5.1 baseline. *Not for production.*

The engine is chosen per application: the `+X-Script-Engine+` header in the application's bundle manifest names it, and an application that declares none runs on the engine set by `xp.script-engine`. Applications on different engines coexist in the same installation.

For what changes for application code — module state, disposers, `executeFunction` — see the https://developer.enonic.com/docs/code/stable/runtime/engines#graaljs[dev kit documentation on JavaScript engines].

==== Tuning the GraalJS context pool

GraalJS executes each application in a *script context*. Contexts are pooled per application and created lazily within an installation-wide budget, so these properties normally need no attention — raise them only when the log reports an exhausted budget. They are ignored on Nashorn.

WARNING: These are preview settings. They may change, or disappear, while GraalJS is in preview.

xp.script-engine.graal.max-contexts:: Total number of script contexts the installation may hold across all applications. Default: `1024`.
xp.script-engine.graal.pool-size:: Context pool capacity per application, clamped to `max-contexts`. Default: the value of `max-contexts`. Above `1`, module-level state is per context - an application that keeps mutable state at module top level sees one copy per context.
xp.script-engine.graal.max-retained-contexts:: Cap on contexts held by live WebSocket and SSE connections, one per connection, so connections cannot consume the whole pool. Default: half of `max-contexts`.
xp.script-engine.graal.max-isolated-contexts:: Cap on contexts created for isolated executions. Default: `1024`.
...
readThrough.cacheCapacity = 1gb
----

baseDir:: specifies root of blobs. Default: `${xp.home}/repo/blob`.

Read-through turns this store into a local cache in front of another blobstore provider - typically a remote object store, where every read that misses locally costs a network round trip. A blob read through the remote provider is written to the local store on the way out, so the next read of it is served from local disk.

readThrough.provider:: name of the blobstore provider to read through to. Default: `none` - no read-through, this store holds the blobs itself.
readThrough.enabled:: enables or disables read-through. Default: `false`.
readThrough.sizeThreshold:: maximum size of an object admitted to the local store. Larger blobs are always served from the read-through provider. Default: `100mb`. The size notation accepts a number plus a byte-size identifier (b/kb/mb/gb/tb/pb).
readThrough.cacheCapacity:: image:xp-810.svg[XP 8.1.0,opts=inline] total size the local store is kept within. Default: `1gb`. The size notation accepts a number plus a byte-size identifier (b/kb/mb/gb/tb/pb).
+
Without a capacity the local store grows to hold everything ever read - which is why `readThrough.cacheCapacity` exists. XP keeps a size-weighted index over the local store and, on every access, decides from it what is admitted and what is evicted.
+
NOTE: On startup the index is seeded from whatever the local store already holds, on a background thread. Until seeding finishes, reads and writes pass straight through without recording accesses.
...
threadPool.virtualThreads = false
threadPool.virtualThreads.maxConcurrent = 1024
...
Configure the security policy for the management Applications API on Port 4848 (`+/server:app/*+`, see <<../endpoints/management/resources#server-app,server:app>>). Controls the `pull` verb (`+/server:app/pull+`, alias `+/server:app/installUrl+`), which installs an application from a remote URL.

This file is the installation-wide policy for `pull`. It applies to every request, whether it arrives on the bare management port or through a virtual host. A virtual host can only withhold `pull` with its <<vhosts#management-api-policy,`verbs`>> setting for `server:app`, never widen it: when `pull` is exposed, the allow-list and checksum requirement below still apply. Per-virtual-host allow-lists are not supported.
...
pull.allowedUrls:: Comma-separated wildcard patterns of URLs `pull` may install from. A trailing `*` matches any suffix; otherwise the pattern is matched exactly as a string prefix. Set to an empty string to disable `pull` entirely. Default: `https://*` (any https URL).

pull.checksumRequired:: When `true`, every `pull` request must include a non-empty `sha512` field; the download is rejected if the computed digest does not match. Set to `false` to allow installs without a checksum (not recommended). Default: `true`.

The legacy keys `installUrl.allowedUrls` and `installUrl.checksumRequired` are still read and have the same meaning. A `pull.*` key takes precedence over its `installUrl.*` counterpart when both are present.

.Restrict installs to the official Enonic public repository
[source,properties]
----
pull.allowedUrls = https://repo.enonic.com/public/*
----

.Allow several explicit hosts
[source,properties]
----
pull.allowedUrls = https://repo.enonic.com/public/*, https://my.cdn.com/apps/*
----

.Disable pull entirely (uploads via /server:app/install are unaffected)
[source,properties]
----
pull.allowedUrls =
----
...
distributable.acceptSystem = true
clustered.timeout = PT10S
----

distributable.acceptInbound:: If true, accept inbound distributable tasks. Default: `true`.
distributable.acceptSystem:: If true, accept inbound distributable tasks belonging to the *system application* - vacuum, audit log cleanup, and the other built-in maintenance tasks, including those the <<#scheduler, scheduler>> submits. Default: `true`.
+
This only narrows what `distributable.acceptInbound` already allows: a node that declines inbound tasks runs no system tasks either, whatever this is set to. Setting it to `false` on its own keeps a node available for application tasks while steering the platform's own housekeeping - which reads and writes across whole repositories - onto other nodes. Set it on the master nodes, alongside <<#scheduler, `acceptScheduling`>>, and for the same reason: a master has to stay responsive to hold the quorum, and this is the heaviest work the platform runs. Send it to a data node without HTTP traffic, or to a node dedicated to background work.
+
WARNING: If no node in a cluster accepts system tasks, they cannot run at all, and submitting one fails.
...
Configure which nodes may run the scheduler, and the jobs to create for scheduling.

acceptScheduling:: image:xp-810.svg[XP 8.1.0,opts=inline] Whether this node may run the scheduler. Default: `true`.
+
Exactly one node of a cluster runs the scheduler, picked among those that allow it, and the duty moves to another such node when that one leaves the cluster. Setting this to `false` excludes the node from that choice; nothing else about it changes - it serves the scheduler API and holds the jobs like every other node, so applications creating or querying jobs behave the same wherever they run.
+
The nodes to exclude are the *master nodes*. Their job is to hold the cluster together - Elasticsearch master election and cluster state, Hazelcast membership - and what that asks of them is responsiveness. A master node can run on modest hardware for exactly that reason. Scheduling, and the heavy maintenance the scheduler submits - vacuum, audit-log cleanup, work that walks whole repositories - is what stops a node answering in time, and a master that stops answering costs the cluster its quorum.
+
Run the schedule where the machine can afford to be busy: a data node that serves no HTTP traffic, or a node dedicated to background work. Excluding the masters is how you steer it there.
+
Has no effect on a non-clustered installation: its single node always runs the scheduler.
+
WARNING: If no node in a cluster allows scheduling, no scheduled job runs at all - including the system jobs. The condition is reported in the log as `Scheduled jobs are not running: no member accepts scheduling`.

.Keeping scheduling off a master node
[source,properties]
----
acceptScheduling = false
----

=== Jobs in the config file

Properties prefixed with `init-job.<my-job-name>` describe a job named `<my-job-name>`. Creation is skipped if a job with that name exists already. Only recurring (cron) jobs can be created from the config file; other schedule types are created through the API.

In a cluster the jobs come from the config file of the node that runs the scheduler. Keep the file consistent across the nodes that allow scheduling.

image:xp-810.svg[XP 8.1.0,opts=inline] What to expect of an edit:

* Adding an entry creates the job within seconds. Nothing restarts, and jobs already running are undisturbed.
* Editing an entry does not change a job that exists. The config file creates jobs, it does not maintain them - modify an existing job through the API, or delete it and let the file create it anew.
* Deleting a job through the API is not undone while the same node keeps the duty. The job may be created again once that node restarts or scheduling moves elsewhere, so remove the entry from the file as well.
* An entry that cannot be read - an invalid descriptor, cron expression or JSON config - is reported in the log and ignored. The other jobs in the file are created as usual.
...
XP ships two jobs of its own. Both are disabled by default, and both are enabled and tuned through the same `init-job.*` properties.

Vacuum:: Deletes unused blobs and binaries from the blobstore. What is considered unused is set in the <<#vacuum, vacuum configuration>>.
+
[source,properties]
----
...
Audit log cleanup:: Deletes records from the audit log repository older than the `ageThreshold` set in `com.enonic.xp.audit.cfg`. Nothing is removed until that threshold is set.
+
[source,properties]
----
...
WARNING: Take a backup before enabling either job. Both delete data that cannot be recovered otherwise.
