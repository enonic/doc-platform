...
xp.script-engine:: image:preview.svg[Preview,opts=inline] The JavaScript engine applications run on, `Nashorn` or `GraalJS`. Default: `Nashorn`. See <<#script-engine, JavaScript engine>> below.
...
For what changes for application code — module state, disposers, `executeFunction` — see the https://developer.enonic.com/docs/code/stable/fundamentals/runtime[dev kit runtime documentation].
...
readThrough.cacheCapacity:: image:xp-8010.svg[XP 8.1.0,opts=inline] total size the local store is kept within. Default: `1gb`. The size notation accepts a number plus a byte-size identifier (b/kb/mb/gb/tb/pb).
+
...
acceptScheduling:: image:xp-8010.svg[XP 8.1.0,opts=inline] Whether this node may run the scheduler. Default: `true`.
+
...
image:xp-8010.svg[XP 8.1.0,opts=inline] What to expect of an edit:
