...
IMPORTANT: Audit log can be disabled by the common `<<logging#audit_enabled, enabled>>` property. Repository flag will not have any effect in this case.
...
Configure the security policy for the management Applications API on Port 4848 (`/server:app/*`). Currently controls the `/installUrl` endpoint, which installs an application from a remote URL.
