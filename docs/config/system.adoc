...
Configure the security policy for the management Applications API on Port 4848 (`+/server:app/*+`, see <<../endpoints/management/resources#server-app,server:app>>). Controls the `pull` verb (`+/server:app/pull+`, alias `+/server:app/installUrl+`), which installs an application from a remote URL. A virtual host exposes or withholds `pull` with its <<vhosts#management-api-policy,`verbs`>> setting for `server:app`.
