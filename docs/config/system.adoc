...
xp.script-engine:: image:preview.svg[Preview,opts=inline] The JavaScript engine applications run on, `Nashorn` or `GraalJS`. Default: `Nashorn`. Any other value stops the installation with `Unsupported script engine`. See <<#script-engine, JavaScript engine>> below.

[#script-engine]
=== JavaScript engine

image:preview.svg[Preview,opts=inline] XP runs server-side JavaScript on one of two engines. *Nashorn* is the default and the supported engine. *GraalJS* ships alongside it as a preview: it implements the current ECMAScript standard rather than Nashorn's ES5.1 baseline, and it caches compiled code across script contexts. *Not for production.*
...
+
With this on, blocking request handling parks a virtual thread instead of holding a platform worker, so slow or script-bound requests cannot starve the pool; selectors and acceptors stay on platform threads, and `threadPool.maxThreads` continues to size them. An option for experiments, not a supported mode.
