...
xp.script-engine:: image:xp-8010.svg[XP 8.1.0,opts=inline] The JavaScript engine applications run on, `Nashorn` or `GraalJS`. Default: `Nashorn`. Any other value stops the installation with `Unsupported script engine`. See <<#script-engine, JavaScript engine>> below.

[#script-engine]
=== JavaScript engine

image:xp-8010.svg[XP 8.1.0,opts=inline] XP runs server-side JavaScript on one of two engines. *Nashorn* is the default and the supported engine. *GraalJS* ships alongside it as a *preview*: it implements the current ECMAScript standard rather than Nashorn's ES5.1 baseline, and it caches compiled code across script contexts. It is not recommended for production yet.

`xp.script-engine` sets the installation-wide default. The engine is decided *per application bundle*, from the `+X-Script-Engine+` manifest header the application declares; an application that declares none runs on the installation default. Applications on different engines coexist in the same installation, so a single app can be moved to GraalJS without touching the rest.

.Run every application on the GraalJS preview
[source,properties]
----
...
+
Without a capacity the local store grows to hold everything ever read - which is why `readThrough.cacheCapacity` exists. XP keeps a size-weighted index over the local store and, on every access, decides from it what is admitted and what is evicted; evicting an entry deletes the blob from the local store. Admission is frequency-based, so a bulk scan - a dump, an export, a re-index - cannot flush the working set: its one-time blobs lose the comparison against established ones and are dropped again immediately.
+
NOTE: On startup the index is seeded from whatever the local store already holds, on a background thread. Until seeding finishes, reads and writes pass straight through without recording accesses and without enforcing the capacity, so the store can briefly exceed it after a restart.
...
threadPool.virtualThreads:: image:xp-8010.svg[XP 8.1.0,opts=inline] *Experimental.* Handle requests on virtual threads. Default: `false`.
+
With this on, blocking request handling parks a virtual thread instead of holding a platform worker, so slow or script-bound requests cannot starve the pool; selectors and acceptors stay on platform threads, and `threadPool.maxThreads` continues to size them. This is an option for experiments, not a supported mode - leave it off in production.
threadPool.virtualThreads.maxConcurrent:: image:xp-8010.svg[XP 8.1.0,opts=inline] Maximum request-handling tasks allowed to run concurrently on virtual threads, so a load spike cannot exhaust memory. `0` or negative means unbounded. Default: `1024`. Keep it well above `threadPool.maxThreads` - bounding it at the platform-thread count would negate the point of virtual threads. Ignored when `threadPool.virtualThreads` is `false`.
...
versionsBatchSize:: Number of node versions the vacuum task reads per batch when walking version history. Default: `10000`. Must not exceed the `+index.max_result_window+` setting of the repository index.
...
+
This narrows `distributable.acceptInbound` rather than widening it: a node that declines inbound tasks runs no system tasks either, whatever this is set to. Setting it to `false` on its own keeps a node available for application tasks while steering the platform's own housekeeping - which reads and writes across whole repositories - onto other nodes. As with <<#scheduler, `acceptScheduling`>>, the typical use is a dedicated master node.
+
...
+
The typical use is a *dedicated master node*. Master nodes are on duty to keep the cluster together - Elasticsearch master election and cluster state, Hazelcast membership - and should be freed from application workload, so that a busy or paused JVM cannot cost the cluster its quorum. Excluding them from scheduling keeps the jobs, and the tasks the scheduler submits, on the data and front-end nodes.
+
...
.Freeing a master node from scheduling
[source,properties]
----
