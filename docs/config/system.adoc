...
NOTE: What a node has created is its own memory, and it starts empty when the node restarts or when scheduling moves to another node. A job deleted through the API while still listed in the config file is created again at that point, as it always was.
