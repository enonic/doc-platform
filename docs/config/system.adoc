...
In a cluster the jobs come from the config file of the node that runs the scheduler. Keep the file consistent across the nodes that allow scheduling, unless the jobs are meant to differ depending on which one has the duty.
