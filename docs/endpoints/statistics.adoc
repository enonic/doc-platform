...
NOTE: The Statistics API is intended for internal monitoring and is by default exposed without authentication on port 2609 - do not expose it publicly as it is. image:xp-8020.svg[XP 8.2.0,opts=inline] A <<../config/vhosts#endpoint, vhost mapping>> may target the statistics endpoint, protecting it with non-interactive ID provider authentication and a principal allow list.
