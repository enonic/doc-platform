...
[NOTE]
====
image:xp-8020.svg[XP 8.2.0,opts=inline] The operations below are also available as <<management/resources#,management resources>> - `+/server:snapshot+`, `+/server:dump+`, `+/server:index+`, ... - a resource-and-verb surface that a <<../config/vhosts#management-api-policy,virtual host can expose selectively>>, and the form new operations are added in. The endpoints on this page keep working unchanged.
====
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:dump` cluster task, on whichever node picks it up; in a cluster `+$XP_HOME/data+` must be on a shared file system.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:upgrade` cluster task.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:load` cluster task.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:export` cluster task, on whichever node picks it up; in a cluster `+$XP_HOME/data+` must be on a shared file system.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:import` cluster task.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:project-sync` cluster task rather than a local task named `sync-all-projects`.
...
WARNING: Deprecated. This call blocks until the reindex completes, which can take hours. Use <<#_reindex_task,Reindex Task>> or `server:index` <<management/resources#server-index,`reindex`>>.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:reindex` cluster task, on whichever node picks it up, rather than on the node that received the request.
...
WARNING: Deprecated as a raw settings passthrough. Replica counts - the setting this is used for - are exposed by `server:index` <<management/resources#server-index,`update`>>; other index settings are internal structure.
...
.Response
https://developer.enonic.com/docs/code/stable/libraries/lib-task#task-info[`TaskInfo`] object.
...
The fields of `calendar` depend on its `type`: `cronValue` and `timeZone` for `CRON`, `value` and image:xp-8010.svg[XP 8.1.0,opts=inline] `deleteAfterRun` for `ONE_TIME`, and image:xp-8010.svg[XP 8.1.0,opts=inline] `duration` - an ISO-8601 duration - for `FIXED_RATE`. Fields that do not apply to the type, and those the job has not set, are left out of the response rather than reported as `null`.
