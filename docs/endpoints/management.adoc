...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Basic authentication is provided by the Standard ID Provider application, for the system ID provider on the management endpoint only. It is enabled by default and can be disabled with `idprovider.system.autologin.basic.enabled = false` in `com.enonic.xp.app.standardidprovider.cfg`.
