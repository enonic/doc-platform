...
[NOTE]
====
image:xp-8020.svg[XP 8.2.0,opts=inline] The operations below are also available as <<management/resources#,management resources>> (`+/server:snapshot+`, `+/server:dump+`, `+/server:index+`, ...), which a <<../config/vhosts#management-api-policy,virtual host exposes selectively>>. The endpoints on this page are unchanged.
====
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:dump` cluster task, on any node of the cluster; in a cluster `+$XP_HOME/data+` must be on a shared file system.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:export` cluster task, on any node of the cluster; in a cluster `+$XP_HOME/data+` must be on a shared file system.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:project-sync` cluster task (previously a local task named `sync-all-projects`).
...
WARNING: Deprecated. Synchronous; use <<#_reindex_task,Reindex Task>> or `server:index` <<management/resources#server-index,`reindex`>>.
...
NOTE: image:xp-8020.svg[XP 8.2.0,opts=inline] Runs as the `com.enonic.xp.app.system:reindex` cluster task, on any node of the cluster.
...
WARNING: Deprecated. Replica settings are exposed by `server:index` <<management/resources#server-index,`update`>>; other index settings are not exposed.
