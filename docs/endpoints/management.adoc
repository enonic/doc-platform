...
image:xp-820.svg[XP 8.2.0,opts=inline] A <<../config/vhosts#endpoint, vhost mapping>> may target the management endpoint, bringing other ID providers (with non-interactive authentication) and a principal allow list to it.
...
NOTE: image:xp-820.svg[XP 8.2.0,opts=inline] Basic authentication is provided by the Standard ID Provider application, for the system ID provider on the management endpoint only. It remains enabled by default for backwards compatibility, and can be disabled with `idprovider.system.autologin.basic.enabled = false` in `com.enonic.xp.app.standardidprovider.cfg` - or per vhost, by giving the ID provider a <<../config/vhosts#flows, flow list>> that does not include `basic`.
