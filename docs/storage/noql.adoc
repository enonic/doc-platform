...
The weight must be an integer; from version 7.16.8 decimal weights (for example ``displayName^1.5``) are also supported, making it possible to scale a field's contribution below the default with a weight between ``0`` and ``1``.
